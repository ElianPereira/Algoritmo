...
universe, persists results to the database, and fires email alerts
...
from app.db.repository import save_daily_summary
from app.db.session import async_session_factory
from app.models.schemas import BatchScreenRequest, DailyScreeningSummary
...
async def run_daily_batch() -> DailyScreeningSummary:
    ...
    3. Persist all results to database
    4. Fire email alerts for qualifying stocks
    5. Persist daily summary
    """
    from app.routers.screening import run_batch_logic
    ...
    logger.info(
        "Daily batch done: %d screened, %d passed filters",
...
def _load_all_tickers() -> list[str]:
    if TICKERS_PATH.exists():
        ...
        return list(dict.fromkeys(
            data.get("sp500", []) + data.get("nyse", []) + data.get("bmv", [])
        ))
